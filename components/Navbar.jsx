...
const Navbar = () => {
    ...
    return (
        <nav className='w-full flex py-6 justify-between items-center navbar'>
            <img src={Clogo} alt='Conatix' className='w-[124px] h-[60px]'/>

            <ul className='list-none sm:flex hidden justify-end items-center flex-1'>
                {navLinks.map((nav, index) => (
                    <li
                        key = {nav.id}
                        className={`font-poppins font-normal cursor-pointer text-[16px] text-neutral-950 ${index == navLinks.length - 1 ? 'mr-0':'mr-10'}`}
                    >
                        <a href={`#${nav.id}`}>
                            {nav.title}
                        </a>
                    ...
                    <div className={`${toggle ? 'flex': 'hidden'} p-6 bg-white absolute top-20 right-0 mx-4 my-2 min-w-[140px] rounded-xl sidebar`}>
                        <ul className='list-none flex flex-col justify-end items-center flex-1'>
                            {navLinks.map((nav, index) => (
                                <li
                                    key = {nav.id}
                                    className={`font-poppins font-normal cursor-pointer text-[16px] text-neutral-950 ${index == navLinks.length - 1 ? 'mr-0':'mb-4'}`}
                                >
                                    <a href={`#${nav.id}`}>
                                        {nav.title}
                                    </a>
